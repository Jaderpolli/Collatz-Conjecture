...
function histograms()
    ...
    for i in 1:length(types)
        ...
        for j in 2:maximumPrimeBlockSize
            ...
            for k in 1:factorial(primeBlockSize)
                fit = readdlm("DATA/AVALANCHES_FIT/fit_avalanche_distribution_n_0_$(k)_$(type)_mVectorSize_$(mVectorSize)_MaxRand_$(MaxRand)_primeBlockSize_$(primeBlockSize).csv", header = false)
        ...
        topPlots = plot!([mean(power_law);mean(power_law)],
            ...
            lw = 1.5,
            lc = :black,
            label = L"\langle\alpha_{av}\rangle =  %$(Float16(average)) \pm %$(Float16(stdDeviation[1]))"
            )
    end
    topPlots = plot!(subplot = 1, ylabel = L"D(\alpha_{av})", annotation = ((0.1,0.9), L"(a)"))
    ...
    bottomPlot = plot!([mean(power_law);mean(power_law)],[0;200],
        ...
        lw = 1.5,
        lc = :black,
        label = label = L"\langle\alpha_{av}\rangle=  %$(Float16(average)) \pm %$(Float16(stdDeviation[1]))"
        )
    ...
    png(plotfinal, string(pasta,"/avalanche_fit_histogram"))
end
